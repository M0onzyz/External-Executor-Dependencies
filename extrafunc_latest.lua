...
getgenv().IS_Sync_LOADED = false
local oldr = request 
getgenv().request = function(options)
	if options.Headers then
    options.Headers["User-Agent"] = "Sync/RobloxApp/2.1"
	else
    options.Headers = {["User-Agent"] = "Sync/RobloxApp/2.1"}
	end
...
getgenv().messagebox = newcclosure(function(text, caption, _type)
    ...
    game:GetService("LinkingService"):OpenUrl(game:GetService("ScriptContext"):SaveScriptProfilingData(ps, "Sync.messagebox.bat"))
...
getgenv().getscriptfunction = function(script)
    ...
    elseif success then
        ...
        return function()
            return nil, "Sync: Script closure access is restricted."
        end
    end
end
...
local __supported_functions_Sync_x64_engine = {
	"request",
	"newcclosure",
...
local __unique_Sync_functions_windows_x64_engine = {
}
...
getgenv().getsupportedfunctions = function()
    return __supported_functions_Sync_x64_engine
end

getgenv().getuniquefunctions = function()
    return __unique_Sync_functions_windows_x64_engine
end
...
getgenv().require = function(scr) -- not mine
    ...
    if not game:GetObjects('rbxassetid://' .. scr)[1] then 
    	warn("Sync: Require failed: invalid asset ID")
    	return 
    end
    if typeof(game:GetObjects('rbxassetid://' .. scr)[1]) == "Instance" and game:GetObjects('rbxassetid://' .. scr)[1].ClassName == "ModuleScript" then
        ...
        if game:GetObjects('rbxassetid://' .. scr)[1].Source ~= "" then 
        	...
        	warn("Sync: Require failed: cant require a modulescript with no code")
        end
    	else 
        warn("Sync: Require failed: require asset id failed")
    	end
    end
    return
	end
...
check("getaffiliateid", function()
    ...
    assert(getgenv().getaffiliateid() == "Sync", "getaffiliateid function test failed")
end)
...
function Bridge:InternalRequest(body, timeout)
    ...
    error("[Sync Error]: Unknown error", 2)
    return
	end
	...
	if success and result then
    error("[Sync Error]: " .. tostring(result), 2)
	end

	error("[Sync Error]: Unknown server error", 2)
end
...
function Bridge:request(options)
	...
	return {
    Success = false,
    StatusMessage = "[Sync Error]: webServer connection failed:  " .. self.serverUrl,
    StatusCode = 599;
...
if not shared.vulnsm then 
    ...
    Bridge:rconsole("cls") 
    rconsolesettitle("Sync is NOT fat!")
	end
...
if not shared.vulnsm then 
    ...
    if type(real) == "userdata" then
        ...
        if table.find(vulnFuncTbl, k) then 
        	return function()
            error("[Sync]: "..tostring(k).." isn't available.")
        	end
