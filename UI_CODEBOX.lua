...
local ENV = {
	...
	-- ── Executor env functions (from your bootstrap) ──────────────────
	getgenv = env.getgenv,
	getrenv = env.getrenv,
	identifyexecutor = env.identifyexecutor,
	getexecutorname = env.identifyexecutor,
	supportluarmour = env.supportluarmour,
	checkcaller = env.checkcaller,
	isexecutorclosure = env.isexecutorclosure,
	checkclosure = env.isexecutorclosure,
	isourclosure = env.isexecutorclosure,
	islclosure = env.islclosure,
	isluaclosure = env.islclosure,
	iscclosure = env.iscclosure,
	newlclosure = env.newlclosure,
	newcclosure = env.newcclosure,
	clonefunction = env.clonefunction,
	hookfunction = env.hookfunction,
	replaceclosure = env.hookfunction,
	hookmetamethod = env.hookmetamethod,
	getrawmetatable = env.getrawmetatable,
	setrawmetatable = env.setrawmetatable,
	...
	-- instance helpers
	getinstances = env.getinstances,
	getnilinstances = env.getnilinstances,
	getloadedmodules = env.getloadedmodules,
	getrunningscripts = env.getrunningscripts,
	getscripts = env.getscripts,
	gethui = env.gethui,
	getscripthash = env.getscripthash,
	getscriptbytecode = env.getscriptbytecode,
	dumpbytecode = env.getscriptbytecode,
	getscriptclosure = env.getscriptclosure,
	getscriptfunction = env.getscriptclosure,
	getsenv = env.getsenv,
	gethiddenproperty = env.gethiddenproperty,
	sethiddenproperty = env.sethiddenproperty,
	isscriptable = env.isscriptable,
	setscriptable = env.setscriptable,

	-- debug extras
	getstack = env.getstack,
	getconnections = env.getconnections,
	getcallbackvalue = env.getcallbackvalue,

	-- thread identity
	getthreadidentity = env.getthreadidentity,
	setthreadidentity = env.setthreadidentity,
	getthreadcontext = env.getthreadidentity,
	setthreadcontext = env.setthreadidentity,
	getidentity = env.getthreadidentity,
	setidentity = env.setthreadidentity,

	-- gc
	getgc = env.getgc,
	filtergc = env.filtergc,

	-- http / requests
	request = env.request,
	http_request = env.request,
	http = env.http,
	HttpGet = env.HttpGet,
	...
	lz4decompress = env.lz4decompress,
	...
	-- input simulation
	mouse1click = env.mouse1click,
	mouse2click = env.mouse2click,
	mouse1press = env.mouse1press,
	mouse1release = env.mouse1release,
	mouse2press = env.mouse2press,
	mouse2release = env.mouse2release,
	mousemoveabs = env.mousemoveabs,
	mousemoverel = env.mousemoverel,
	mousescroll = env.mousescroll,

	-- clipboard
	setclipboard = env.setclipboard,
	toclipboard = env.setclipboard,
	setrbxclipboard = env.setclipboard,

	-- console
	consolecreate = env.consolecreate,
	rconsolecreate = env.consolecreate,
	consoleclear = env.consoleclear,
	rconsoleclear = env.consoleclear,
	consoleprint = env.consoleprint,
	rconsoleprint = env.consoleprint,
	consolewarn = env.consolewarn,
	rconsolewarn = env.consolewarn,
	consoleerr = env.consoleerr,
	rconsoleerr = env.consoleerr,
	rconsoleerror = env.consoleerr,
	consoleinput = env.consoleinput,
	rconsoleinput = env.consoleinput,
	rconsoleclose = env.rconsoleclose,
	rconsoledestroy = env.rconsoleclose,
	consoledestroy = env.rconsoleclose,
	rconsolesettitle = env.rconsolesettitle,
	consolesettitle = env.rconsolesettitle,
	rconsolename = env.rconsolesettitle,

	-- misc executor
	messagebox = env.messagebox,
	isrbxactive = env.isrbxactive,
	isgameactive = env.isrbxactive,
	closeroblox = env.closeroblox,
	setfpscap = env.setfpscap,
	setfps = env.setfpscap,
	getfpscap = env.getfpscap,
	getfps = env.getfpscap,
	queue_on_teleport = env.queue_on_teleport,
	queueonteleport = env.queue_on_teleport,
	run_on_actor = env.run_on_actor,
	runonactor = env.run_on_actor,
	createactor = env.createactor,
	getactors = env.getactors,
	getallactors = env.getallactors,
	firesignal = env.firesignal,
	fireallconnections = env.firesignal,
	...
	firetouchtransmitter = env.firetouchinterest,
	invalidateinstance = env.invalidateinstance,
	isinstancecached = env.isinstancecached,
	compile = env.compile,
	setscriptbytecode = env.setscriptbytecode,
	setglobal = env.setglobal,
...
Lib.CodeFrame = (function()
	...
	local updateAutocompletes; function updateAutocompletes(obj)
		...
		autocompleteType = word and word[4] or 0
					...
					if not quickExist[item] then
						...
						if similarity > 0.4 then
							...
							if not isDeprecated then
								table.insert(list, { similarity, item, autocompleteTypes[cat], false, cat })
							else
								table.insert(deprecated, { similarity, item, autocompleteTypes[cat], true, cat })
							end
						end
					end
				end
	...
	local function setupEditBox(obj)
		...
		editBox:GetPropertyChangedSignal("Text"):Connect(function()
			...
			if obj.EditBoxCopying or obj.FocusIgnore or uis.TouchEnabled and obj.LastTyped == obj.FramesPassed then
			...
			if #originalText > 1 then
				...
				if (originalText == "\n" or originalText == "\t") and obj.Autocompleting and word ~= "" and autocompleteIndex ~= 0 then
					...
					obj.Lines[obj.CursorY + 1] = textWithoutWord .. word .. (autocompleteType == 2 and "()" or "") .. textAfterCursor

					text = text:sub(1, -2)

					obj:MoveCursor(obj.CursorX + (#word - skips + (autocompleteType == 2 and 1 or 0)), obj.CursorY)
					word = ""
				...
				if obj.ControlButtons then
					...
					if tb and tb:IsDescendantOf(frame) and isDown("Control") then
						...
						elseif input.KeyCode == Enum.KeyCode.F then
							obj.Goto.Visible = false
							obj.Replace.Visible = not obj.Replace.Visible or tb ~= objects.Instance3
						...
						elseif input.KeyCode == Enum.KeyCode.H or input.KeyCode == Enum.KeyCode.R then
							obj.Goto.Visible = false
							obj.Replace.Visible = not obj.Replace.Visible or tb ~= objects.Instance3 and tb ~= objects.Instance6

							if obj.Replace.Visible then
								if objects.Instance3.Text == "" then
									objects.Instance3:CaptureFocus()
								else
	...
	funcs.UpdateCursor = function(self,input)
		...
		if cursorVisible then
			...
			cursor:TweenPosition(UDim2.new(0, (self.LinesVisible and linesOffset or 8) + offX * math.ceil(self.FontSize / self.Colors.WidthDivider), 0, offY * self.FontSize), Enum.EasingDirection.Out, Enum.EasingStyle.Sine, 0.1, true)
	...
	funcs.CoverSelection = function(self, symbol, symbolEnd)
		if not funcs:IsValidRange() then
			return
		end

		if not symbolEnd then
			symbolEnd = symbol
		end

		local lines = self.Lines
		local lineStart, lineEnd = lines[self.SelectionRange[1][2] + 1], lines[self.SelectionRange[2][2] + 1]

		lines[self.SelectionRange[1][2] + 1] = lineStart:sub(1, self.SelectionRange[1][1] + 1) .. symbol .. lineStart:sub(self.SelectionRange[1][1] + 1)
		lines[self.SelectionRange[2][2] + 1] = lineEnd:sub(1, self.SelectionRange[2][1] + 1) .. symbolEnd .. lineEnd:sub(self.SelectionRange[2][1] + 1)

		self.SelectionRange[1][1] += 1
		self.SelectionRange[2][1] += (self.SelectionRange[1][2] == self.SelectionRange[2][2] and 2 or 0)
		self.CursorX, self.CursorY = self.SelectionRange[2][1], self.SelectionRange[2][2]
	...
	funcs.UpdateView = function(self)
		...
		if oldOffsets ~= self.FrameOffsets then
			...
			self.Frame.Lines.Position = UDim2.new(0,self.LinesVisible and linesOffset or 8,0,0)
			self.Frame.Lines.Size = UDim2.new(1,self.LinesVisible and -linesOffset+oldOffsets.X or -linesOffset,1,oldOffsets.Y)
			...
			self.Frame.LineNumbers.Visible = self.LinesVisible
		end
	end
